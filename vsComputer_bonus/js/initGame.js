...
async function startGame(){
	...
	name2 = 'csd bot';
	let selected_color = document.getElementById('p2').value;

	if(name1.length < 1){ //dont proceed until both players have entered a name
		document.getElementById('error').innerHTML =  'Enter you names';
		return ;
	}
	...
	player_r  = new Player(selected_color, name1);
	if(selected_color == 'Red')
		player_wh = new Player('White', name2);
	else
		player_wh = new Player('Red', name2);
	setOpponents();
	setStartingPlayer();
	updateInfoBox('Game Started!','Roll the dice to play!');

	if(getPlayerTurn().name == 'csd bot') {  //if bot gets to play first

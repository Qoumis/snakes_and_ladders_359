...
function movePlayer(){
    ...
    if(cells[new_pos].type == 'pythonEffect')   //Python cell scenario
        ...
        msg2 = getPlayerTurn().color;
       if(!getPlayerTurn().pyeffect){
           msg2 += ' player landed on snake cell No' + new_pos;
           new_pos = cells[new_pos].to;
           msg2 += ' and fell down to cell No' + new_pos;
       }
       else
           msg2 += ' player landed on a snake cell, but since they have python effect, snakes wont touch them!';
    }

    else if(cells[new_pos].type == 'Ladders'){  //Ladder cell scenario
        msg2 = getPlayerTurn().color;
        msg2 += ' player landed on ladder cell No' + new_pos;
        new_pos = cells[new_pos].to;
